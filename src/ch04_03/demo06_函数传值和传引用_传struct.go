...
func main() {
	a := Teacher{"kevin", 42, true, 1}
	fmt.Printf("1、变量a的内存地址：%p,值为：%v\n\n", &a, a) //{kevin,42,true,1}

	//传值,特别注意，切片slice是引用类型，传值实际传递的也是地址
	changeStructVal(a)
	fmt.Printf("2、changeStructVal调用后：变量a的内存地址：%p,值为：%v\n\n", &a, a) //{kevin,42,true,1}

	//传引用
	changeStructPtr(&a)
	fmt.Printf("3、changeStructPtr调用后：变量a的内存地址：%p,值为：%v\n\n", &a, a) //{tom,42,false,1}
}

func changeStructVal(a Teacher) {
	fmt.Printf("----------changeStructVal函数内：参数a的内存地址：%p,值为%v\n", &a, a) //{kevin,42,true,1}
	a.name = "jason"
	a.age = 34
}

func changeStructPtr(a *Teacher) {
	fmt.Printf("----------changeStructPtr函数内：参数a的内存地址：%p,值为%v\n", &a, a) //&{kevin,42,true,1}
	(*a).name = "tom"
	(*a).married = false
